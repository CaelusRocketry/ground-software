...
const ButtonPane = () => {
  ...
  const actuateValve = (loc, type, p) => {
    ...
    if (
      !window.confirm(
        "Are you sure you want to actuate the " +
          type +
          " valve at " +
          loc +
          " w/ priority " +
  ...
  const request = (type, objectType, location) => {
    ...
    dispatch(
      requestPressed({ type: type, objectType: objectType, location: location })
    );
  };
  ...
  const progress = () => {
    ...
    if (
      !window.confirm(
        "Are you sure you want to progress to " +
          stage_names[stages[current_stage + 1]] +
          "?"
      )
    ) {
      return;
      ...
      <div class={views.actuation ? "block mt-2" : "hidden"}>
        <div class="mt-2">
          ...
          {createSelect(
            ...
            "actuationType",
            [4, 3, 2, 1],
            acutation_type_names
          )}
          {createSelect("Priority", "actuationPriority", [1, 2, 3], {
            1: 1,
            2: 2,
            3: 3,
          })}
