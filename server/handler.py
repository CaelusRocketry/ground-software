        ...
        while self.running:
            try:
                if self.queue_send and SEND_ALLOWED:
                    ...
                    subpackets = [packet_str[i:60+i] for i in range(0, len(packet_str), 60)] #split into smaller packets of 255

                    for subpacket in subpackets:
                        self.ser.write(subpacket)
                      

                    print("\nSent packet:", packet_str, len(packet_str))
